...
namespace UnityGameFramework.Runtime
{
    ...
    public sealed class EntityGroup
    {
        ...
        private readonly GameFrameworkLinkedList<Entity> m_Entities;
        ...
        /// <param name="root">实体组根节点。</param>
        /// <param name="objectPoolManager">对象池管理器。</param>
        internal EntityGroup(string name, float instanceAutoReleaseInterval, int instanceCapacity, float instanceExpireTime, int instancePriority, Transform root, IObjectPoolManager objectPoolManager)
        {
            ...
            m_Name = name;
            m_Root = root;
            m_InstancePool = objectPoolManager.CreateSingleSpawnObjectPool<EntityInstanceObject>(Utility.Text.Format("Entity Instance Pool ({0})", name), capacity: instanceCapacity, expireTime: instanceExpireTime, priority: instancePriority);
            m_InstancePool.AutoReleaseInterval = instanceAutoReleaseInterval;
            m_Entities = new GameFrameworkLinkedList<Entity>();
            m_CachedNode = null;
        }
        ...
        internal void RegisterEntityInstanceObject(EntityInstanceObject obj, bool spawned)
        {
            m_InstancePool.Register(obj, spawned);
        }

        internal EntityInstanceObject SpawnEntityInstanceObject(string name)
        {
            return m_InstancePool.Spawn(name);
        }

        internal void UnspawnEntity(Entity entity)
        {
            UnspawnEntityInstance(entity.Handle);
        }

        internal void UnspawnEntityInstance(object entityInstance)
        {
            ...
            m_InstancePool.Unspawn(entityInstance);
        }

        internal void SetEntityInstanceLocked(object entityInstance, bool locked)
        {
            if (entityInstance == null)
            {
                throw new GameFrameworkException("Entity instance is invalid.");
        ...
        internal void SetEntityInstancePriority(object entityInstance, int priority)
        {
            if (entityInstance == null)
            {
                throw new GameFrameworkException("Entity instance is invalid.");
